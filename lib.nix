...
let inherit (pkgs.lib) evalModules;
in {
  ...
  mkNeovim = { config }:
    let
      ...
      opts = evalModules {
        modules = [ { imports = [ ./plugins ]; } config ];
  ...
  buildPluginOverlay = super: self:
    let
      ...
      inherit (builtins) attrNames filter getAttr listToAttrs;
      ...
      buildPlug = name:
        buildVimPluginFrom2Nix {
          pname = name;
          version =
            last (splitString "/" (attrByPath [ name "url" ] "HEAD" inputs));
