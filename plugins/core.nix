...
in {
            ...
            type = types.listOf (types.submodule ({ ... }: {
              options = {
                ...
                cmd = mkOption {
                  ...
                  type = types.str;
                  description =
                    "Defines the command to be executed in AUto Command";
                  internal = true;
                };
              };
            }));
  ...
  config.nvim = let
    ...
    mapOpts = o:
      concatStrings
      (flatten [ "{" (mapAttrsFlatten (k: v: "${k} = ${boolStr v},") o) "}" ]);
    concatKeybinding = m: k: v: o:
      concatStrings [
        "map("
        ''"${m}",''
        ''"${k}",''
    ...
    mapLuabinding = mode: maps: o:
      mapAttrsFlatten (k: v: concatKeybinding mode k v o) maps;
    ...
    mapAutocmd = { pattern, cmd, name, }:
      ...
      '';
    mapAugroup = { name, clear, autocmds, }:
      let groupName = pat: concatStringsSep "_" [ (toLower pat) "group" ];
      in ''
